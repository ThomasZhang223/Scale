...
import { runScanThumbJobs } from "./lib/scan-thumb";
import { postCatalogIngest } from "./lib/catalog-ingest";
import { shopifyRoute } from "./lib/shopify-route";
...
async function dispatch(
  ...
  if (pathname.startsWith('/v1/shopify/')) return shopifyRoute(req, env);
  ...
  if (m("POST", /^\/v1\/intent$/)) return real.postIntent(req, env);
...
export default {
  ...
  async scheduled(_event: ScheduledController, env: Env): Promise<void> {
    // The outbox first, always. It is a handful of D1 statements and one Durable Object call,
    // while a thumbnail render waits on a headless browser for up to 27 s — so the cheap,
    // latency-sensitive relay must never queue behind a slow browser. Its failure is still
    // raised, after the render has had its turn.
    let failure: unknown;
    try {
