...
export default {
  ...
  /**
   * Queue consumer: the unattended catalog pre-bake.
   *
   * The queue exists to throttle, not to transport. The free plan allows 100 concurrent
   * Workflow instances, and a 60-100 product pre-bake started in a loop would exhaust that and
   * push the demo's own live generation into the back of the line. `max_concurrency = 3` in
   * wrangler.toml is the whole point of this indirection.
   */
  async queue(batch: MessageBatch<{ objectId: string; tier: "live" | "quality"; apiOrigin: string }>, env: Env): Promise<void> {
      ...
      try {
        ...
        await env.DB.prepare(
          `INSERT INTO jobs (id, object_id, kind, tier, state, progress_pct, error, created_at, updated_at)
           VALUES (?, ?, 'mesh', ?, 'queued', 0, NULL, ?, ?)`,
        )
          .bind(jobId, objectId, tier, new Date().toISOString(), new Date().toISOString())
          .run();
        await env.GENERATE_MESH.create({
          id: jobId,
          params: { jobId, objectId, tier, apiOrigin, roomId: null },
        });
        message.ack();
      } catch {
        // Retry with backoff rather than acking. A pre-bake that fails is worth another go;
        // wrangler.toml caps it at max_retries = 3 so a poisoned message cannot loop forever.
        message.retry();
      }
    }
  },
