"""Solver tests: the 10 from docs/agent/02_LAYOUT_SOLVER.md plus S1–S4 from 09.
...
os.environ.setdefault("SOLVER_KEY", "test-key")
...
class Http(unittest.TestCase):
    ...
    def test_S4_health_reports_ortools(self):
        res = self.client.get("/health", headers={"X-Solver-Key": "test-key"})
        self.assertEqual(res.status_code, 200)
        self.assertTrue(res.json()["ortools"].startswith("9.15"))

    def test_solve_over_http(self):
        res = self.client.post("/solve", json=request([]), headers={"X-Solver-Key": "test-key"})
        self.assertEqual(res.status_code, 200)
